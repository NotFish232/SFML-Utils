...
private:
    ...
    std::function<Entity *(const std::string &)> m_getEntityCallback;
    bool m_visible;
...
public:
    ...
    virtual const std::vector<FloatRect> getBounds() const;

    bool isVisible() const;
    const std::string &getName() const;
    void setSignalCallback(const std::function<void(const std::string &)> &func);
    void setGetEntityCallback(const std::function<Entity *(const std::string &)> &func);
